...
public class ImportPassageService {
  ...
  public long importer(
    ...
    try {
      ...
      try (PreparedStatement psPass = conn.prepareStatement(sqlPassage,
          Statement.RETURN_GENERATED_KEYS)) {

        psPass.setString(1, numeroCarre);
        psPass.setString(2, codePoint);
        psPass.setInt(3, numeroPassage);
        psPass.setInt(4, annee);

        psPass.executeUpdate();

        try (ResultSet keys = psPass.getGeneratedKeys()) {
          if (!keys.next()) {
            throw new SQLException("Aucune clé générée pour le passage.");
          }
          passageId = keys.getLong(1);
        }
      }

      try (PreparedStatement psObs = conn.prepareStatement(sqlObservation)) {

        for (ObservationAImporter observation : observations) {

          psObs.setLong(1, passageId);
          psObs.setDouble(2, observation.tempsDebut());
          psObs.setDouble(3, observation.tempsFin());
          psObs.setDouble(4, observation.frequenceMediane());
          psObs.setString(5, observation.codeTaxon());
          psObs.setDouble(6, observation.probabilite());
      ...
      throw new DataAccessException(
          "Impossible d'importer le passage", e);

    } finally {
      fermerSilencieusement(conn);
    }
  }
